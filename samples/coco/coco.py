...
import shutil
from samples.shapes import train_shapes
...
if __name__ == '__main__':
    ...
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'evaluate' or 'train new data'on MS COCO")
    ...
    if args.command == "train":
        ...
        model.train(dataset_train, dataset_val,
                    ...
                    augmentation=augmentation)
        """
    elif args.command == "train new data":
        # Training dataset
        dataset_train = train_shapes.ShapesDataset()
        dataset_train.load_shapes(500, config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1])
        dataset_train.prepare()

        # Validation dataset
        dataset_val = train_shapes.ShapesDataset()
        dataset_val.load_shapes(50, config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1])
        dataset_val.prepare()

        augmentation = imgaug.augmenters.Fliplr(0.5)

        print("Training network heads with new data")
        model.train(dataset_train, dataset_val,
                    learning_rate=config.LEARNING_RATE,
                    epochs=40,
                    layers='heads',
